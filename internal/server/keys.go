...
type keymap struct {
	...
	Commands      string       `yaml:"commands"`       // opens the saved-commands picker/manager
	NewCommand    string       `yaml:"new_command"`    // inside the commands picker, opens the add-command form — a "mod+key" spec (like Prefix/Lock), since plain letters there type into the search box
	EditCommand   string       `yaml:"edit_command"`   // inside the commands picker, edits the highlighted command — a "mod+key" spec, same reason
	DeleteCommand string       `yaml:"delete_command"` // inside the commands picker, deletes the highlighted command — a "mod+key" spec, same reason
